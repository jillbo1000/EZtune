...
svm.reg.hjn <- function(x = x, y = y, cross = cross, fast = fast, loss = loss) {
  ...
  if(is.null(cross) & !fast) {
    fit <- function(x) {svm.reg.opt.resub(x, dat, loss)}
  } else if(fast > 0) {
    if(fast > 1) {
      n <- fast
    } else if(fast < 1) {
    ...
    fit <- function(x) {svm.reg.opt.fast(x, dat, n, loss)}
    results$n <- n
  } else if(!is.null(cross)) {
    if(cross >= 2) {
      fit <- function(x) {svm.reg.opt.cv(x, dat, cross, loss)}
    } else {
