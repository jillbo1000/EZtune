...
en.reg.ga <- function(x = x, y = y, cross = cross, fast = fast, loss = loss) {
  ...
  if(is.null(cross) & !fast) {
    fit <- function(p) {-1 * en.reg.opt.resub(p, x, y, loss)}
  } else if (fast > 0) {
    if(fast > 1) {
      n <- fast
    } else if(fast < 1) {
    ...
    fit <- function(p) {-1 * en.reg.opt.fast(p, x, y, n, loss)}
    results$n <- n
  } else if(!is.null(cross)) {
    if(cross >= 2) {
      fit <- function(p) {-1 * en.reg.opt.cv(p, x, y, cross, loss)}
    } else {
  ...
  ga.obj <- GA::ga(type = "real-valued", fitness = fit, parallel = 2,
                   maxiter = 10, run = 5, lower = c(0, lmin), upper = c(1, lmax))

  results$alpha <- ga.obj@solution[1, 1]
  results$lambda <- exp(ga.obj@solution[1, 2])
  results$loss <- -1 * as.numeric(ga.obj@fitnessValue)
